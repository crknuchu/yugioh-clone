...
enum class CardType { MONSTER_CARD, SPELL_CARD, TRAP_CARD };
...
enum class MonsterPosition { ATTACK, FACE_UP_DEFENSE, FACE_DOWN_DEFENSE, NONE };

enum class SpellTrapPosition { SET, FACE_UP, NONE };
...
public:
  ...
  virtual void setCardMenu(bool isMonsterZoneFull,
                           bool OpponentHaveMonsters) = 0;
  ...
  bool faceUp = true; // card is turned face up (image is visible) or facedown
                      // (back is visible)
  ...
  static const std::map<SpellTrapPosition, QString>
      spellTrapPositionEnumToQString;

  static const std::map<QString, MonsterPosition> monsterPositionQStringToEnum;
  static const std::map<QString, SpellTrapPosition>
      spellTrapPositionQStringToEnum;

protected:
  // Constructor is protected in order to restrict call to it to Card's child
  // classes.
  Card(const std::string &cardName, CardType cardType,
