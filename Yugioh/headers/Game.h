...
private:
  ...
  void battleBetweenTwoAttackPositionMonsters(MonsterCard &attacker,
                                              MonsterCard &defender);
  void battleBetweenTwoDifferentPositionMonsters(MonsterCard &attacker,
                                                 MonsterCard &defender);
  ...
  QTcpSocket *m_pTcpSocket =
      nullptr; // TODO: This will probably have to be in GameExternVars so that
               // EffectActivator can see it
  qint32 m_clientID;
  QString m_clientName;
  QDataStream m_inDataStream;
  QString m_currentHeader;
  static const std::map<QString, DESERIALIZATION_MEMBER_FUNCTION_POINTER>
      m_deserializationMap;

  bool sendDataToServer(QByteArray &data);
  void notifyServerThatDeserializationHasFinished();
  QByteArray QInt32ToQByteArray(
      qint32 source); // We use qint32 to ensure the number has 4 bytes
