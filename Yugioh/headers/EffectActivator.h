...
class EffectActivator : public QObject {
  ...
  // Our type that represents a pointer to EffectActivator member function that accept no arguments
  // and return void
...
private:
  ...
  // TODO: Does every method here need a targetPlayer in order to differentiate whose card we need
  // to check
  void makeMonstersOfThisTypeUntargetable(const MonsterType& type,
                                          const UntargetableBy& forbiddenMechanic,
  ...
  std::vector<MonsterCard*> findLowestATKMonsters(
      Player& targetPlayer); // MonsterCard* because we want a pointer to the actual cards
  ...
  /* This could be void and instead emit a signal that would color the avaliable face-up monsters.
     Or it could return a vector like it does now, and then we can do the above in the card that
     calls this. */
