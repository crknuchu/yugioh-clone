...
#include <iterator>
#include <numeric>
Card::Card(const std::string &cardName, CardType cardType,
           CardLocation cardLocation, const std::string &cardDescription,
           std::string imagePath)
    : imagePath(imagePath), cardName(cardName), cardType(cardType),
      cardLocation(cardLocation), cardDescription(cardDescription) {
  QPixmap pixmap;
  pixmap.load(QString::fromStdString(imagePath));
  pixmap = pixmap.scaled(
      QSize(200, 150),
      Qt::KeepAspectRatio); // pixmap size needs to not be hardcoded
...
auto Card::getCardLocationString() const -> std::string {
  ...
  case CardLocation::GRAVEYARD: return "graveyard";
  case CardLocation::FIELD: return "field";
  default: return "error:unsupported card location";
  }
}
...
auto Card::getCardTypeString() const -> std::string {
  switch (cardType) {
  case CardType::MONSTER_CARD: return "monster card";
  case CardType::TRAP_CARD: return "trap card";
  case CardType::SPELL_CARD: return "spell card";
  default: return "error: unsupported card type";
  }
}
...
void Card::hoverEnterEvent(QGraphicsSceneHoverEvent *event) {
  // TODO: Is this even possible since QGraphicsPixmapItem doesn't inherit
  // QObject
...
void Card::move(float x, float y) {
  setPos(x, y); // we need to implement a unified move() function that moves the
                // card and the menu at the same time
...
auto operator<<(std::ostream &out, Card &c) -> std::ostream & {
  return out << "Card name: " << c.getCardName()
             << ", card type: " << c.getCardTypeString()
