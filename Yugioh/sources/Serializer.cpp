  ...
  for (auto item : arr) {
    ...
    if (item.toObject()["type"].toString() == "Normal Monster" ||
      ...
      const std::string imagePath =
          item.toObject()["image"].toString().toStdString();
      int atk = item.toObject()["atk"].toInt();
      int def = item.toObject()["def"].toInt();
      int level = item.toObject()["level"].toInt();
      MonsterKind race =
          Maps::stringToMonsterKind[item.toObject()["race"].toString()]; // kind
      MonsterAttribute attribute =
          Maps::stringToMonsterAttribute[item.toObject()["attribute"]
                                             .toString()];
      MonsterType type =
          Maps::stringToMonsterType[item.toObject()["archetype"].toString()];

      // Placeholder
      MonsterPosition position = MonsterPosition::NONE;

      auto *monsterCard = new MonsterCard(
          name, atk, def, level, type, race, attribute, false, position, false,
      ...
      const std::string imagePath =
          item.toObject()["image"].toString().toStdString();
      SpellType spellType =
          Maps::stringToSpellType[item.toObject()["race"].toString()];

      // Placeholder
      SpellTrapPosition position = SpellTrapPosition::NONE;

      auto *spellCard =
          new SpellCard(spellType, name, cardType, CardLocation::DECK, position,
                        desc, imagePath, false);
      ...
      const std::string imagePath =
          item.toObject()["image"].toString().toStdString();
      TrapType trapType =
          Maps::stringToTrapType[item.toObject()["race"].toString()];

      // Placeholder
      SpellTrapPosition position = SpellTrapPosition::NONE;

      auto *trapCard =
          new TrapCard(trapType, name, cardType, CardLocation::DECK, position,
                       desc, imagePath, false, false);
