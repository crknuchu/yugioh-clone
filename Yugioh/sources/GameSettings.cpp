...
#include <iostream>
GameSettings::GameSettings(QWidget* parent) : QDialog(parent), ui(new Ui::GameSettings) {
  ui->setupUi(this);
...
const QMap<TimePerMove, int> GameSettings::getTimePerMoveEnumToInt{{TimePerMove::MINIMAL_TIME, 3},
                                                                   {TimePerMove::SMALLER_TIME, 5},
                                                                   {TimePerMove::STANDARD_TIME, 6},
                                                                   {TimePerMove::BIGGER_TIME, 10},
                                                                   {TimePerMove::MAXIMUM_TIME, 20}};

const QMap<Decks, QString> GameSettings::getDeckEnumToString{{Decks::KAIBA, "KAIBA"},
                                                             {Decks::YUGI, "YUGI"}};
...
void GameSettings::saveGameSettingsJson(int lifePoints, int numberOfCards, int timePerMove,
                                        QString deck) {
  QJsonObject obj;
...
void GameSettings::setLifePoints(LifePoints newLifePoints) { lifePoints = newLifePoints; }

auto GameSettings::getNumberOfCards() const -> NumberOfCards { return numberOfCards; }
...
void GameSettings::setTimePerMove(TimePerMove newTimePerMove) { timePerMove = newTimePerMove; }
...
void GameSettings::on_okButton_clicked() {
  ...
  saveGameSettingsJson(getLifePointsEnumToInt[this->lifePoints],
                       ...
                       getTimePerMoveEnumToInt[this->timePerMove], getDeckEnumToString[this->deck]);
  close();
}
