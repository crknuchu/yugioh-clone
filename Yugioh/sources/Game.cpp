...
const std::map<QString, Game::DESERIALIZATION_MEMBER_FUNCTION_POINTER>
    Game::m_deserializationMap
    = { { "START_GAME", &Game::deserializeStartGame },
        ...
        { "DESERIALIZATION_FINISHED",
          &Game::deserializeDeserializationFinished },
...
Game::Game (Player p1, Player p2, int lifePoints, int numberOfCards,
            int timePerMove, QWidget *parent)
    : QMainWindow (parent), lifePoints (lifePoints),
      numberOfCards (numberOfCards), timePerMove (timePerMove),
      ui (new Ui::MainWindow), m_player1 (p1), m_player2 (p2),
...

{
  ...
  /* Install an event filter for Resize event
     ...
     standard way of this->size() would return window size from the form
     designer, and not after going fullscreen. However, this only applies to
     this constructor (in other parts, this->size() can be used).
  */
...
auto Game::getTimePerMove () const -> int { return timePerMove; }
...
auto Game::getNumberOfCards () const -> int { return numberOfCards; }
...
auto Game::getLifePoints () const -> int { return lifePoints; }

void Game::setLifePoints (int newLifePoints) { lifePoints = newLifePoints; }
...
void Game::switchPlayers ()
{
  ...
  std::cout << "Current player is: " << *GameExternVars::pCurrentPlayer
            << std::endl;
  ui->labelCurrentPlayerNameDynamic->setText (QString::fromStdString (
      GameExternVars::pCurrentPlayer->getPlayerName ()));
  ui->labelOtherPlayerNameDynamic->setText (
      QString::fromStdString (GameExternVars::pOtherPlayer->getPlayerName ()));

  // Switch clients too
  GameExternVars::currentTurnClientID == 1
      ? GameExternVars::currentTurnClientID = 2
      : GameExternVars::currentTurnClientID = 1;

  // Notify the server about the new turn beginning:
  QEventLoop blockingLoop;
  connect (this, &Game::deserializationFinished, &blockingLoop,
           &QEventLoop::quit);
  QByteArray buffer;
  QDataStream outDataStream (&buffer, QIODevice::WriteOnly);
  outDataStream.setVersion (QDataStream::Qt_5_15);
  outDataStream
      << QString::fromStdString ("NEW_TURN")
      << GameExternVars::currentTurnClientID; // We tell the opposite player
                                              // that his opponent (current
                                              // player in this call) got a
                                              // card
...
void Game::damageCalculation (Card *attackingMonster, Card *attackedMonster)
{
  ...
  if (defender->getPosition () == MonsterPosition::ATTACK)
    ...
    {
      std::cout << "Battle between 2 different position monsters begins!"
                << std::endl;
...
void Game::battleBetweenTwoAttackPositionMonsters (MonsterCard &attacker,
                                                   MonsterCard &defender)
{
  /* TODO: We can also send the name of player who initiated the attack to the
     server, but its always the current player so maybe its not needed. */

  int attackPointsDifference
      = attacker.getAttackPoints () - defender.getAttackPoints ();
  if (attackPointsDifference < 0)
    {
      ...
      qint32 attackerZoneNumber
          = findZoneNumber (attacker, GameExternVars::pCurrentPlayer);

      // Find the zone that holds the destroyed card
      Zone *pZoneOfTheDestroyedCard = findZone (
          attackerZoneNumber, QString::fromStdString ("monster card"),
          GameExternVars::pCurrentPlayer);
      Card *pDestroyedCard = pZoneOfTheDestroyedCard->m_pCard;

      // Actually destroy it
      GameExternVars::pCurrentPlayer->sendToGraveyard (
          *pDestroyedCard, pZoneOfTheDestroyedCard);
      ...
      outDataStream
          << QString::fromStdString (
                 "DESTROY_CARD") // Notify the server that a card was destroyed
          << QString::fromStdString (
                 "CURRENT_PLAYER") // Who owns a card that was destroyed
    ...
    {
      //        damagePlayer(*GameExternVars::pOtherPlayer,
      //        attackPointsDifference);
      std::cout << "The attacker wins!" << std::endl;

      /* We want to notify the server about the battle outcome.
       * We also have to use QEventLoop to make this client wait until client 2
       * sends DESERIALIZATION_FINISHED (which will get sent to this client by
       * the server) by making it loop until its built-in slot quit() is
       *  triggered by our deserializationFinished signal.
       *
       *  IMPORTANT: In theory its possible that deserializationFinished is
       * emitted before we enter loop.exec(), which would leave us in an
       * infinite loop, but that will practically never happen since that
       * signal is emitted after a lot of other operations (send to server ->
       * relay to another client -> send back -> emit) so we will always enter
       * the loop before that.
       *
       *  TODO: We can make sure that this won't happen by using singleshot
       * QTimer instead of a standard emit call */
      ...
      qint32 defenderZoneNumber
          = findZoneNumber (defender, GameExternVars::pOtherPlayer);

      // TODO: These 3 lines can be a separate function since they are reused a
      // lot in these battle functions We need to find zone of the destroyed
      // card
      Zone *pZoneOfTheDestroyedCard = findZone (
          defenderZoneNumber, QString::fromStdString ("monster card"),
          GameExternVars::pOtherPlayer);
      Card *pDestroyedCard = pZoneOfTheDestroyedCard->m_pCard;

      // Actually destroy it
      GameExternVars::pOtherPlayer->sendToGraveyard (*pDestroyedCard,
                                                     pZoneOfTheDestroyedCard);
      ...
      outDataStream
          << QString::fromStdString (
                 "DESTROY_CARD") // Notify the server that a card was destroyed
          << QString::fromStdString (
                 "OTHER_PLAYER") // Who owns a card that was destroyed
  ...
  else
    {
      ...
      connect (this, &Game::deserializationFinished, &blockingLoopAttacker,
               &QEventLoop::quit);
      ...
      connect (this, &Game::deserializationFinished, &blockingLoopDefender,
               &QEventLoop::quit);

      qint32 defenderZoneNumber
          = findZoneNumber (defender, GameExternVars::pOtherPlayer);

      pZoneOfTheDestroyedCard = findZone (
          defenderZoneNumber, QString::fromStdString ("monster card"),
          GameExternVars::pOtherPlayer);
...
void Game::battleBetweenTwoDifferentPositionMonsters (MonsterCard &attacker,
                                                      MonsterCard &defender)
{
  ...
  if (defender.getPosition () == MonsterPosition::FACE_DOWN_DEFENSE)
    {
      ...
      connect (this, &Game::deserializationFinished, &blockingLoop1,
               &QEventLoop::quit);
      QByteArray buffer1;
      QDataStream outDataStream1 (&buffer1, QIODevice::WriteOnly);
      outDataStream1.setVersion (QDataStream::Qt_5_15);
      outDataStream1
          << QString::fromStdString ("FLIP")
          << QString::fromStdString ("OTHER_PLAYER")
          << QString::fromStdString (
                 defender.getCardName ()) // TODO: This is maybe unneeded,
                                          // since we can locate the monster by
                                          // just using the zone number
      ...
      bool cardActivationReq
          = effectRequirement.isActivatable (defender.getCardName ());
      if (cardActivationReq)
        {
          ...
          connect (this, &Game::deserializationFinished, &blockingLoop2,
                   &QEventLoop::quit);
          QByteArray buffer2;
          QDataStream outDataStream2 (&buffer2, QIODevice::WriteOnly);
          outDataStream2.setVersion (QDataStream::Qt_5_15);
          outDataStream2
              << QString::fromStdString ("EFFECT_ACTIVATED")
              << QString::fromStdString ("OTHER_PLAYER")
              << QString::fromStdString (
                     defender.getCardName ()) // Who activated the effect
              << QString::fromStdString (defender.getCardTypeString ())
              << defenderZoneNumber;
  ...
  if (pointsDifference < 0)
    {
       ...
       * get destroyed but the player controlling the attacker still takes
       * damage. */
      std::cout << "The defender wins!" << std::endl;
    ...
    {
      /* If the attacker was stronger, the defender gets destroyed but the
       * player that was controlling the defender doesn't take damage because
       * it was in DEFENSE position. */
      std::cout << "The attacker wins!" << std::endl;
      QEventLoop blockingLoop;
      connect (this, &Game::deserializationFinished, &blockingLoop,
               &QEventLoop::quit);
...
auto Game::findZone (qint32 zoneNumber, QString cardType,
                     Player *pTargetPlayer) -> Zone *
{
...
void Game::visuallySetMonster (MonsterCard *pMonsterCard)
{
  ...
  if (pMonsterCard->getPosition () == MonsterPosition::FACE_DOWN_DEFENSE)
    {
      ...
      pix = pix.scaled (QSize (pMonsterCard->width, pMonsterCard->height),
                        Qt::KeepAspectRatio);
    }
  ...
  pMonsterCard->move (pMonsterCard->x () - pMonsterCard->width / 5,
                      pMonsterCard->y () + pMonsterCard->height / 5);
}
...
void Game::visuallySetSpell (SpellCard *pSpellCard)
{
  QPixmap pix;
  pix.load (":/resources/pictures/card_back.jpg");
  pix = pix.scaled (QSize (pSpellCard->width, pSpellCard->height),
                    Qt::KeepAspectRatio);
...
void Game::visuallySetTrap (TrapCard *pTrapCard)
{
  QPixmap pix;
  pix.load (":/resources/pictures/card_back.jpg");
  pix = pix.scaled (QSize (pTrapCard->width, pTrapCard->height),
                    Qt::KeepAspectRatio);
...
{
  // FIXME: We can't use this for both flipping our and opponent's cards, since
  // our cards will be rotated by 180 degress which is something we don't want
  // to do.
  QPixmap pix;
                           ...
                           float windowWidth, float windowHeight)
{
  ...
  std::cout << "Window width/height in firstTurnSetup: " << windowWidth
  ...
  std::cout << "The first one to play is "
            << GameExternVars::pCurrentPlayer->getPlayerName () << std::endl;
  m_currentTurn = 1;

  GamePhaseExternVars::currentGamePhase = GamePhases::DRAW_PHASE;
  ui->labelGamePhase->setText (
      QString::fromStdString ("DRAW PHASE")); // We hardcode it here since both
                                              // clients are in sync here
  ...
  if (m_clientID == firstToPlay)
    {
      ...
      for (Card *card : GameExternVars::pOtherPlayer->m_hand.getHand ())
        {
          cardBackPixmap = cardBackPixmap.scaled (
              QSize (card->width, card->height), Qt::KeepAspectRatio);
  ...
  //                                               MonsterKind::EFFECT_MONSTER,
  //                                               MonsterAttribute::EARTH,
  //                                               false,
  //                                               MonsterPosition::FACE_DOWN_DEFENSE,
  //                                               false,
  //                                               CardType::MONSTER_CARD,
  //                                               CardLocation::FIELD,
  //                                               "Neither player can target
  ...
  //                                               );
  //     SpellCard* testCard4 = new SpellCard(SpellType::NORMAL_SPELL, "Card
  //     Destruction",
  //                                             CardType::SPELL_CARD,
  //                                             CardLocation::HAND,
  //                                             SpellTrapPosition::NONE, "  An
  //                                             EARTH monster equipped with
  //                                             this card increases " "its ATK
  //                                             by 400 points and decreases
  //                                             its DEF by 200 points.",
  //                                             ":/resources/pictures/CardDestruction.jpg",
  //                                             true);
  ...
  //   //testing purposes
  //   GameExternVars::pCurrentPlayer->field.monsterZone.placeInMonsterZone(testCard1,
  //   2); visuallySetMonster(testCard1);
  ...
  //                                               ":/resources/pictures/HaneHane.jpg"
  //                                               );
  //   SpellCard* testCard2 = new SpellCard(SpellType::NORMAL_SPELL, "Change of
  //   Heart",
  //                                              CardType::SPELL_CARD,
  //                                              CardLocation::HAND, "
  //                                              Increase your Life Points by
  //                                              1000 points.  ",
  //                                              ":/resources/pictures/ChangeofHeart.jpg",
  //                                              true);
  ...
  //                                               );
  //   SpellCard* testCard4 = new SpellCard(SpellType::NORMAL_SPELL, "Monster
  //   Reborn",
  //                                              CardType::SPELL_CARD,
  //                                              CardLocation::HAND, "  An
  //                                              EARTH monster equipped with
  //                                              this card increases " "its
  //                                              ATK by 400 points and
  //                                              decreases its DEF by 200
  //                                              points.",
  //                                        ":/resources/pictures/MonsterReborn.jpg",
  //                                        true);
  ...
  //   GameExternVars::pCurrentPlayer->m_hand.addToHand(*testCard2);
  //   GameExternVars::pCurrentPlayer->m_hand.addToHand(*testCard4);
  //  GameExternVars::pCurrentPlayer->field.graveyard->sendToGraveyard(new
  //  testCard);
  // Label setup:
  ui->labelCurrentPlayerNameDynamic->setText (QString::fromStdString (
      GameExternVars::pCurrentPlayer->getPlayerName ()));
  ...
  ui->labelCurrentPlayerLpDynamic->setText (
      QString::fromStdString (std::to_string (
          GameExternVars::pCurrentPlayer->getPlayerLifePoints ())));
  ui->labelOtherPlayerLpDynamic->setText (QString::fromStdString (
      std::to_string (GameExternVars::pOtherPlayer->getPlayerLifePoints ())));

  ui->progressBar->setValue (
      GameExternVars::pCurrentPlayer->getPlayerLifePoints ());
  ui->enemyPoints->setValue (
      GameExternVars::pOtherPlayer->getPlayerLifePoints ());
}
...
void Game::setupConnections ()
{
  ...
  connect (&m_player1, &Player::cardAddedToScene, this,
           &Game::onCardAddedToScene);
  connect (&m_player2, &Player::cardAddedToScene, this,
           &Game::onCardAddedToScene);
  connect (this, &Game::gameEndedAfterBattle, this,
           &Game::onGameEnd); // Same slot for both game endings (one in
                              // EffectActivator and one here)
  ...
  // Buttons
  connect (ui->btnBattlePhase, &QPushButton::clicked, this,
           &Game::onBattlePhaseButtonClick);
  connect (ui->btnMainPhase2, &QPushButton::clicked, this,
           &Game::onMainPhase2ButtonClick);
  connect (ui->btnEndPhase, &QPushButton::clicked, this,
           &Game::onEndPhaseButtonClick);
  // Networking
  connect (m_pTcpSocket, &QIODevice::readyRead, this, &Game::onDataIncoming);
  connect (m_pTcpSocket, &::QAbstractSocket::errorOccurred, this,
           &Game::onNetworkErrorOccurred);
  connect (ui->btnConnect, &QPushButton::clicked, this,
           &Game::onConnectButtonClick);
  connect (ui->btnWriteData, &QPushButton::clicked, this,
           &Game::onWriteDataButtonClick);

  connect (this, &Game::returnCardToOpponent, this,
           &Game::onReturnCardToOpponent);
}
  ...
  if (event->type () == QEvent::Resize)
    {
      ...
      // We need to emit the signal here so that we can scale the UI AFTER we
      // catch it
...
auto Game::sendDataToServer (QByteArray &data) -> bool
{
  if (m_pTcpSocket->state () == QAbstractSocket::ConnectedState)
    ...
    {
      std::cerr << "Error: the socket that is trying to write the data is in "
                   "UNCONNECTED state!"
                << std::endl;
      return false;
    }
...
void Game::deserializeStartGame (QDataStream &deserializationStream)
{
  ...
  deserializationStream >> firstToPlay >> clientID;
...
{
  // TODO: There are still things here to be done, like reconstructing the
  // cards and summoning them in correct zone and position

  QString cardName;
  QString cardType;
  qint32 zoneNumber;
  QString positionQString;
  deserializationStream >> cardName >> cardType >> zoneNumber
      >> positionQString;
  ...
  std::cout << "Card position: " << positionQString.toStdString ()
            << std::endl;

  // TODO: Here we will recreate the card in the future and put it in the
  // correct zone.
  Card *targetCard;
  for (Card *card : GameExternVars::pCurrentPlayer->m_hand.getHand ())
    {
  ...
  for (Card *card :
       GameExternVars::pCurrentPlayer->field.graveyard->getGraveyard ())
    {
      if (cardName.toStdString () == card->getCardName ())
        {
          targetCard = card;
          GameExternVars::pCurrentPlayer->field.graveyard
              ->removeFromGraveyard (*targetCard);
        }
    }
  ...
  if (cardType == "monster card")
    {
      ...
      MonsterPosition monsterPosition
          = monsterCard->monsterPositionQStringToEnum.at (positionQString);
      ...
      GameExternVars::pCurrentPlayer->field.monsterZone.placeInMonsterZone (
          targetCard, zoneNumber);
    }
  else
    {
      ...
      if (cardType == "spell card")
        {
          ...
          stPosition
              = spellCard->spellTrapPositionQStringToEnum.at (positionQString);
      ...
      else
        {
          ...
          stPosition
              = trapCard->spellTrapPositionQStringToEnum.at (positionQString);
          trapCard->setPosition (stPosition);
          if (stPosition == SpellTrapPosition::SET)
            visuallySetTrap (trapCard);
        }

      GameExternVars::pCurrentPlayer->field.spellTrapZone
          .placeInSpellTrapZone (targetCard, zoneNumber);
    }

  // Rotate it by 180 degrees to be facing towards us
  QTransform transformationMatrix;
  transformationMatrix.rotate (180);
  targetCard->setPixmap (
      targetCard->pixmap ().transformed (transformationMatrix));
...
{
  /* When player1 adds a card to the hand, game sends a header to the server to
     indicate that, and then we come here. In order for player2 (who is
     actually the CURRENT player while we are in this function) to changes in
     the opponent's (player1) hand, we just need to make other player draw one
     card.

     TODO: We need to somehow ensure that, for example player1's, decks are in
     sync, in both game sessions/clients.       !!

      // We don't want this player to see the opponent's hand, so we could
     potentially change the pixmap to card_back.jpg
  */
  ...
  whoGetsTheCards == QString::fromStdString ("CURRENT_PLAYER")
      ? GameExternVars::pCurrentPlayer->drawCards (numOfCards)
      : GameExternVars::pOtherPlayer->drawCards (numOfCards);
...
void Game::deserializeLpChange (QDataStream &deserializationStream)
{
  ...
  deserializationStream >> whoseLifePointsChanged >> newLifePoints;
  // Now we need to actually set the targeted player's lp to newLifePoints
  if (whoseLifePointsChanged.toStdString ()
      == GameExternVars::pCurrentPlayer->getPlayerName ())
    {
  ...
  else
    {
      ...
      ui->labelOtherPlayerLpDynamic->setText (
          QString::fromStdString (std::to_string (
              GameExternVars::pOtherPlayer->getPlayerLifePoints ())));
    }
...
void Game::deserializeDeserializationFinished (
    QDataStream &deserializationStream)
{
  Q_UNUSED (deserializationStream);
...
void Game::deserializeGamePhaseChanged (QDataStream &deserializationStream)
{
  ...
  GamePhaseExternVars::currentGamePhase
      = GamePhaseExternVars::QStringToGamePhase.at (currentGamePhaseQString);
...
void Game::deserializeNewTurn (QDataStream &deserializationStream)
{
  ...
  /* TODO: This can be a separate function since same code is used in
     switchPlayers(), yet we can't call switchPlayers because we don't want to
     send a NEW_TURN header */
  Player *tmp = GameExternVars::pCurrentPlayer;
  GameExternVars::pCurrentPlayer = GameExternVars::pOtherPlayer;
  GameExternVars::pOtherPlayer = tmp;
...
void Game::deserializeEffectActivated (QDataStream &deserializationStream)
{
  ...
  QString cardType;
  qint32 zoneNumber;
  deserializationStream >> whichPlayerActivatedEffect >> cardName >> cardType
      >> zoneNumber;
  ...
  if (whichPlayerActivatedEffect == QString::fromStdString ("CURRENT_PLAYER"))
    {
      cardType == "monster card"
          ? targetCard = GameExternVars::pCurrentPlayer->field.monsterZone
                             .m_monsterZone[zoneNumber - 1]
                             ->m_pCard
          : targetCard = GameExternVars::pCurrentPlayer->field.spellTrapZone
                             .m_spellTrapZone[zoneNumber - 1]
                             ->m_pCard;
    }
  else
    {
      cardType == "monster card"
          ? targetCard = GameExternVars::pOtherPlayer->field.monsterZone
                             .m_monsterZone[zoneNumber - 1]
                             ->m_pCard
          : targetCard = GameExternVars::pOtherPlayer->field.spellTrapZone
                             .m_spellTrapZone[zoneNumber - 1]
                             ->m_pCard;
    }

  EffectActivator effectActivator (*targetCard);
  connect (&effectActivator, &EffectActivator::lifePointsChanged, this,
           &Game::onLifePointsChange);
  connect (&effectActivator, &EffectActivator::gameEnded, this,
           &Game::onGameEnd);
...
void Game::deserializeReposition (QDataStream &deserializationStream)
{
  QString cardName;
  qint32 zoneNumber;
  deserializationStream >> cardName >> zoneNumber;

  Card *targetCard = GameExternVars::pCurrentPlayer->field.monsterZone
                         .m_monsterZone[zoneNumber - 1]
                         ->m_pCard;
...
void Game::deserializeFlip (QDataStream &deserializationStream)
{
  ...
  qint32 zoneNumber;
  deserializationStream >> whichPlayerFlipped >> cardName >> zoneNumber;
...
void Game::deserializeDestroyCard (QDataStream &deserializationStream)
{
  ...
  deserializationStream >> whoOwnedTheDestroyedCardQString >> cardType
      >> zoneNumber;
  ...
  // Find the zone that holds the destroyed card
  Zone *pZoneOfTheDestroyedCard
      = findZone (zoneNumber, cardType, pPlayerWhoOwnedTheDestroyedCard);
  Card *pDestroyedCard = pZoneOfTheDestroyedCard->m_pCard;

  // If the card was set, we need to make it be face up (so it won't be
  // face-down in the graveyard)
  if (cardType == QString::fromStdString ("monster card"))
    {
      ...
      if (position == MonsterPosition::FACE_UP_DEFENSE
  ...
  // Actually destroy it
  pPlayerWhoOwnedTheDestroyedCard->sendToGraveyard (
      *pDestroyedCard,
      pZoneOfTheDestroyedCard); // TODO: Change all other occurences and add
                                // zone parameter
...
void Game::onGameStart (qint32 firstToPlay, qint32 clientID)
{
  ...
  std::cout << "First one to play is client with id " << firstToPlay
            << std::endl;
  ...
  clientID == firstToPlay
      ? m_clientName = QString::fromStdString (
            GameExternVars::pCurrentPlayer->getPlayerName ())
      : m_clientName = QString::fromStdString (
            GameExternVars::pOtherPlayer->getPlayerName ());
  ...
  GameExternVars::pCurrentPlayer->field.setField (
      flagBottomField, m_viewAndSceneWidth, m_windowHeight);
  GameExternVars::pCurrentPlayer->m_hand.setHandCoordinates (
      m_viewAndSceneWidth, m_windowHeight, flagBottomField);
  for (auto zone :
       GameExternVars::pCurrentPlayer->field.monsterZone.m_monsterZone)
    {
      ...
      connect (zone, &Zone::zoneGreenAndClicked, this,
               &Game::onGreenZoneClick);
      ui->graphicsView->scene ()->addItem (zone);
    }
  for (auto zone :
       GameExternVars::pCurrentPlayer->field.spellTrapZone.m_spellTrapZone)
    {
      connect (zone, &Zone::zoneRedAndClicked, this, &Game::onRedZoneClick);
      connect (zone, &Zone::zoneGreenAndClicked, this,
               &Game::onGreenZoneClick);
  ...
  ui->graphicsView->scene ()->addItem (
      GameExternVars::pCurrentPlayer->field.graveyard);
  ui->graphicsView->scene ()->addItem (
      GameExternVars::pCurrentPlayer->field.fieldZone);

  GameExternVars::pOtherPlayer->field.setField (
      flagUpperField, m_viewAndSceneWidth, m_windowHeight);
  GameExternVars::pOtherPlayer->m_hand.setHandCoordinates (
      m_viewAndSceneWidth, m_windowHeight, flagUpperField);
  for (auto zone :
  ...
  for (auto zone :
       GameExternVars::pOtherPlayer->field.spellTrapZone.m_spellTrapZone)
    {
      connect (zone, &Zone::zoneRedAndClicked, this, &Game::onRedZoneClick);
      connect (zone, &Zone::zoneGreenAndClicked, this,
               &Game::onGreenZoneClick);
  ...
  ui->graphicsView->scene ()->addItem (
      GameExternVars::pOtherPlayer->field.graveyard);
  ui->graphicsView->scene ()->addItem (
      GameExternVars::pOtherPlayer->field.fieldZone);
...
void Game::onEndPhaseButtonClick ()
{
  ...
  for (Zone *zone :
    ...
    {
      if (!zone->isEmpty ()
          && zone->m_pCard->getCardType () == CardType::TRAP_CARD)
        {
  ...
  // Something may happen here, like checking the effects that resolve in the
  // end phase
...
void Game::onGamePhaseChange (const GamePhases &newGamePhase)
{
  ...
  ui->labelGamePhase->setText (
      GamePhaseExternVars::gamePhaseToQString.at (newGamePhase));

  QString currentGamePhase = GamePhaseExternVars::gamePhaseToQString.at (
      GamePhaseExternVars::currentGamePhase);
  std::cout << "Current game phase in onGamePhaseChange: "
            << currentGamePhase.toStdString () << std::endl;
...
void Game::onTurnEnd ()
{
  ...
  Card *cardThatWeGotJustNow
      = GameExternVars::pCurrentPlayer->m_hand.getHand ().back ();

  if (m_clientID != GameExternVars::currentTurnClientID)
    {
      QPixmap cardBackPixmap (":/resources/pictures/card_back.jpg");
      cardBackPixmap = cardBackPixmap.scaled (
          QSize (cardThatWeGotJustNow->width, cardThatWeGotJustNow->height),
          Qt::KeepAspectRatio);
  ...
  QDataStream outDataStream (&buffer, QIODevice::WriteOnly);
  outDataStream.setVersion (QDataStream::Qt_5_15);
  outDataStream << QString::fromStdString ("ADD_CARD_TO_HAND") << qint32 (1)
  ...
  /* ... Something may happen here due to effects (maybe we should call
   checkEffects() or something similar that will check if there are effects to
   be activated in SP */
...
/* In order to have drag resizes on the main window, we can place CentralWidget
   in a layout, but weird things tend to happen when we actually resize then,
   so for now its not done like that.*/
void Game::onMainWindowResize (QResizeEvent *resizeEvent)
{
  /* TODO: Is there a way to make Qt only do one resize (immediately to the
     fullscreen resolution, instead of first resizing to the resolution that is
     same as in Designer (and only after that one doing a proper resize to
     fullscreen) */
  ...
  /* Initial setup (we need 2nd resize because that is the one when the window
     goes fullscreen) Here we initialize objects, etc. (so that they don't get
     initialized on every resize event) */
  if (resizeCount == 2)
    {
      ...
      // Check: Very rarely, this displays the same width/height as the old
      // window std::cout << "New main window width/height: " << m_windowWidth
      ...
      // We need to calculate other UI sizes so we know what QGraphicsView's
      // size needs to be.
      // TODO: Change leftVerticalLayout name to something normal
      const int leftVerticalLayoutWidth
          = ui->leftVerticalLayout->sizeHint ().width ();

      m_viewAndSceneWidth = m_windowWidth - (leftVerticalLayoutWidth);
      ui->graphicsView->setFixedSize (m_viewAndSceneWidth, m_windowHeight);
      ui->graphicsView->scene ()->setSceneRect (0, 0, m_viewAndSceneWidth,
                                                m_windowHeight);

      // TODO: Check if this is needed
      ui->graphicsView->fitInView (0, 0, m_viewAndSceneWidth, m_windowHeight,
                                   Qt::KeepAspectRatio);

      QPixmap background (":/resources/pictures/space.jpeg");
      background = background.scaled (
          m_viewAndSceneWidth, this->size ().height (), Qt::IgnoreAspectRatio);
        ...
        case (avatars::YUGI):
          {
            ...
            ui->namePlayer->setText (
                QString::fromStdString (this->m_player1.getPlayerName ()));
            break;
          }
        case (avatars::KAIBA):
          {
      ...
      ui->enemyName->setText (
          QString::fromStdString (this->m_player2.getPlayerName ()));
...
{
  std::cout << "Card " << card.getCardName () << " hover-entered!"
            << std::endl;

  QString cardPosition;

  /* If the card is a monster, we want its ATK/DEF in card description too.
   * If its not a monster, it will be empty string and won't mess with the
   * description. */
  ...
  if (card.getCardType () == CardType::MONSTER_CARD)
    {
      ...
      atkDefIfMonster
          += "ATK: " + std::to_string (pMonsterCard->getAttackPoints ())
             + "  /  DEF: "
             + std::to_string (pMonsterCard->getDefensePoints ());

      cardPosition = MonsterCard::monsterPositionEnumToQString.at (
          pMonsterCard->getPosition ());
    }
  else if (card.getCardType () == CardType::SPELL_CARD)
    {
      auto *pSpellCard = static_cast<SpellCard *> (&card);

      cardPosition = SpellCard::spellTrapPositionEnumToQString.at (
          pSpellCard->getSpellPosition ());
    }
  else
    {
      auto *pTrapCard = static_cast<TrapCard *> (&card);

      cardPosition = TrapCard::spellTrapPositionEnumToQString.at (
          pTrapCard->getTrapPosition ());
    }
  ...
  if (card.y () >= m_windowHeight / 2
      || (card.y () < m_windowHeight / 2 && cardPosition != "FACE_DOWN_DEFENSE"
          && cardPosition != "SET"))
    {
...
void Game::onCardSelect (Card *card)
{
  ...
  // We need to check if the opponent has any monsters. If he doesn't, we don't
  // show Attack Directly button
  ...
  // We only want current client to be able to see card menus, and only on his
  // own cards
  if (m_clientID == GameExternVars::currentTurnClientID
      && card->y () >= m_windowHeight / 2)
    card->cardMenu->isVisible () == false ? card->cardMenu->show ()
                                          : card->cardMenu->hide ();
}
  ...
  if (activatedSpellCard->getSpellType () == SpellType::FIELD_SPELL)
    {
      ...
      GameExternVars::pCurrentPlayer->field.fieldZone->putInZone (
          activatedSpellCard);
    }
  else
...
{
  std::cout << "Activate button clicked on card " << card.getCardName ()
            << std::endl;
  ...
  if (card.getCardLocation () == CardLocation::HAND)
  ...
  else
    {
      ...
      qint32 zoneNumber
          = findZoneNumber (card, GameExternVars::pCurrentPlayer);
      ...
      outDataStream << QString::fromStdString ("EFFECT_ACTIVATED")
                    << QString::fromStdString ("CURRENT_PLAYER")
                    << QString::fromStdString (
                           cardName) // Who activated the effect
      ...
      connect (&effectActivator, &EffectActivator::effectMonsterReborn, this,
               &Game::onMonsterReborn);
      connect (&effectActivator, &EffectActivator::effectChangeOfHeart, this,
               &Game::onChangeOfHeart);

      if (card.shouldBeSentToGraveyard ())
        { // this needs to be implemented
          ...
          QDataStream outDataStream2 (&buffer2, QIODevice::WriteOnly);
          outDataStream2.setVersion (QDataStream::Qt_5_15);
          outDataStream2 << QString::fromStdString (
              "DESTROY_CARD") // Notify the server that a card was destroyed
                         << QString::fromStdString (
                                "CURRENT_PLAYER") // Who owns a card that was
                                                  // destroyed
                         << QString::fromStdString (card.getCardTypeString ())
          ...
          for (Zone *zone1 : current.field.monsterZone.m_monsterZone)
            {
              if (zone1->isEmpty () && flag == false)
                {
                  current.field.monsterZone.placeInMonsterZone (zone->m_pCard,
                                                                zone1);
                  flag = true;
...
{
  std::cout << "Summon button was clicked on card " << card.getCardName ()
            << std::endl;

  // Remove target card from player's hand
  GameExternVars::pCurrentPlayer->m_hand.removeFromHand (card);

  /* Set this card that is to-be-summoned to a global summon target, in order
     for Zone objects to be able to see it. */
  GameExternVars::pCardToBePlacedOnField = &card;
  std::cout << "Current summon target is: "
            << GameExternVars::pCardToBePlacedOnField->getCardName ()
            << std::endl;

  // Set the monster's position explicitly to ATTACK (since thats the only one
  // allowed when summoning)
...
{
  std::cout << "Reposition button clicked for card " << card.getCardName ()
            << std::endl;

  // We are sure that this card is a MonsterCard since only monsters can change
  // their position to defense or attack
  auto *monsterCard = static_cast<MonsterCard *> (&card);
  ...
  qint32 zoneNumber
      = findZoneNumber (*monsterCard, GameExternVars::pCurrentPlayer);
  ...
  outDataStream.setVersion (QDataStream::Qt_5_15);
  outDataStream
      << QString::fromStdString ("REPOSITION")
      << QString::fromStdString (
             monsterCard->getCardName ()) // TODO: This is maybe unneeded,
                                          // since we can locate the monster by
                                          // just using the zone number
...
void Game::onFlipButtonClick (Card &card)
{
  ...
  bool cardActivationRequirement
      = effectRequirement.isActivatable (monsterCard->getCardName ());
...
/* TODO: This should accept the targetPlayer as an argument, because otherwise
   we always print currentPlayer's hp, even if it was other player's health
   that had changed. */
void Game::onLifePointsChange (Player &targetPlayer) // const?
{
  std::cout << "Current health points for player "
            << targetPlayer.getPlayerName () << " : "
            << targetPlayer.getPlayerLifePoints () << std::endl;
  ...
  outDataStream.setVersion (QDataStream::Qt_5_15);
  outDataStream
      << QString::fromStdString ("LP_CHANGE")
      << QString::fromStdString (
             targetPlayer.getPlayerName ()) // Whose life points has changed
...
void Game::onGameEnd (Player &loser)
{
  ...
  ui->labelOtherPlayerLpDynamic->setText (QString::fromStdString (
      std::to_string (GameExternVars::pOtherPlayer->getPlayerLifePoints ())));
  std::cout << "The game has ended! Player " << loser.getPlayerName ()
            << " has lost because his health points reached 0 !" << std::endl;
  ...
  outDataStream << QString::fromStdString ("LP_CHANGE")
                << QString::fromStdString (
                       loser.getPlayerName ()) // Whose life points has changed
                << qint32 (0);                 // New life points
  sendDataToServer (buffer);
  blockingLoop.exec ();

  // Display a pop-up window for both players that the game has ended and tell
  // them the result.
  ...
  QDataStream outDataStream2 (&buffer2, QIODevice::WriteOnly);
  outDataStream2.setVersion (QDataStream::Qt_5_15);
  outDataStream2 << QString::fromStdString ("GAME_END") << loserName;
  sendDataToServer (buffer2);
  blockingLoop2.exec ();
...
{
  std::cout << "Set button clicked on card " << card.getCardName ()
            << std::endl;
  ...
  /* Set this card that is to-be-summoned to a global summon target, in order
     for Zone objects to be able to see it. */
  GameExternVars::pCardToBePlacedOnField = &card;
  std::cout << "Current Set target is: "
            << GameExternVars::pCardToBePlacedOnField->getCardName ()
            << std::endl;

  /* Since both spells/traps and monsters can be Set (but the behavior
     differs!), we need to see what this card actually is. */
...
// TODO: summonAlready = true , activatedAlready = true at the end of
// onRedZoneClick?
  ...
  if (card->getCardType () == CardType::MONSTER_CARD)
    {
      ...
      auto *pMonsterCard = static_cast<MonsterCard *> (card);
      QString monsterPosition = pMonsterCard->monsterPositionEnumToQString.at (
          pMonsterCard->getPosition ());

      // If the position is SET, we need to rotate the card and change the
      // pixmap to card_back.jpg
      ...
      outDataStream.setVersion (QDataStream::Qt_5_15);
      outDataStream
          << QString ("FIELD_PLACEMENT") // Header, so that server knows what
                                         // data to expect after it
          << QString::fromStdString (
                 card->getCardName ()) // We only send card's name, server will
                                       // pass it to the other client and then
                                       // that client will construct the card
          << QString::fromStdString (card->getCardTypeString ()) << zoneNumber
          << monsterPosition;
      sendDataToServer (buffer);
      blockingLoop.exec ();
    ...
    {
      GameExternVars::pCurrentPlayer->field.spellTrapZone
          .placeInSpellTrapZone (card, clickedRedZone);
      card->setCardLocation (CardLocation::FIELD);
      ...
      if (card->getCardType () == CardType::SPELL_CARD)
        {
          auto *pSpellCard = static_cast<SpellCard *> (card);
          spellTrapPosition = pSpellCard->spellTrapPositionEnumToQString.at (
              pSpellCard->getSpellPosition ());
        ...
        {
          auto *pTrapCard = static_cast<TrapCard *> (card);
          spellTrapPosition = pTrapCard->spellTrapPositionEnumToQString.at (
              pTrapCard->getTrapPosition ());
...
void Game::onGreenZoneClick (Zone *clickedGreenZone)
{
  ...
  GameExternVars::pAttackingMonster->cardMenu->attackButton->setVisible (
      false);
...
void Game::onDataIncoming ()
{
  ...
  auto deserializationFunctionPointer
      = m_deserializationMap.at (m_currentHeader);
...
{
  // Testing
  std::cout << "Current player draws 5 - not actually, this is a test"
            << std::endl;
  QEventLoop blockingLoop;
  connect (this, &Game::deserializationFinished, &blockingLoop,
           &QEventLoop::quit);
  ...
  outDataStream << QString::fromStdString ("ADD_CARD_TO_HAND") << qint32 (5)
                << QString::fromStdString ("CURRENT_PLAYER");
...
void Game::onCardAddedToScene (Card *card)
{
  ...
  connect (card->cardMenu->activateButton, &QPushButton::clicked, this,
           [this, card] () { onActivateButtonClick (*card); });
  connect (card->cardMenu->setButton, &QPushButton::clicked, this,
           [this, card] () { onSetButtonClick (*card); });
  connect (card->cardMenu->summonButton, &QPushButton::clicked, this,
           [this, card] () { onSummonButtonClick (*card); });
  connect (card->cardMenu->attackButton, &QPushButton::clicked, this,
           [this, card] () { onAttackButtonClick (*card); });
  connect (card->cardMenu->repositionButton, &QPushButton::clicked, this,
           [this, card] () { onRepositionButtonClick (*card); });
  connect (card->cardMenu->attackDirectlyButton, &QPushButton::clicked, this,
           [this, card] () { onAttackDirectlyButtonClick (*card); });
  connect (card->cardMenu->flipButton, &QPushButton::clicked, this,
           [this, card] () { onFlipButtonClick (*card); });
  ...
  for (Zone *zone :
       GameExternVars::pCurrentPlayer->field.monsterZone.m_monsterZone)
    {
      if (zone->m_pCard == GameExternVars::pCardToBePlacedOnField)
        {
          GameExternVars::pCurrentPlayer->field.monsterZone
  ...
  for (Zone *zone :
       GameExternVars::pOtherPlayer->field.monsterZone.m_monsterZone)
    {
      if (zone->isEmpty ())
        {
          GameExternVars::pOtherPlayer->field.graveyard->removeFromGraveyard (
              *GameExternVars::pCardToBeReturned);
          GameExternVars::pOtherPlayer->field.monsterZone.placeInMonsterZone (
              GameExternVars::pCardToBeReturned, zone);
