...
Game::Game(Player p1, Player p2, int lifePoints, int numberOfCards,
    ...
    : QMainWindow(parent), lifePoints(lifePoints), numberOfCards(numberOfCards),
      timePerMove(timePerMove), ui(new Ui::MainWindow), m_player1(p1),
      m_player2(p2), m_pTcpSocket(new QTcpSocket(this))
...
void Game::setTimePerMove(int newTimePerMove) { timePerMove = newTimePerMove; }
...
void Game::switchPlayers() {
  ...
  std::cout << "Current player is: " << *GameExternVars::pCurrentPlayer
            << std::endl;
  ui->labelCurrentPlayerNameDynamic->setText(
      QString::fromStdString(GameExternVars::pCurrentPlayer->getPlayerName()));
  ...
  outDataStream
      ...
      << GameExternVars::currentTurnClientID; // We tell the opposite player
                                              ...
                                              // player in this call) got a card
...
void Game::battleBetweenTwoAttackPositionMonsters(MonsterCard &attacker,
  ...
  if (attackPointsDifference < 0) {
    ...
    Zone *pZoneOfTheDestroyedCard =
        findZone(attackerZoneNumber, QString::fromStdString("monster card"),
                 GameExternVars::pCurrentPlayer);
    Card *pDestroyedCard = pZoneOfTheDestroyedCard->m_pCard;

    // Actually destroy it
    GameExternVars::pCurrentPlayer->sendToGraveyard(*pDestroyedCard,
     ...
     * sends DESERIALIZATION_FINISHED (which will get sent to this client by the
     * server) by making it loop until its built-in slot quit() is triggered by
     * our deserializationFinished signal.
     *
     *  IMPORTANT: In theory its possible that deserializationFinished is
     * emitted before we enter loop.exec(), which would leave us in an infinite
     * loop, but that will practically never happen since that signal is emitted
     * after a lot of other operations (send to server -> relay to another
     * client -> send back -> emit) so we will always enter the loop before
     * that.
     *
     *  TODO: We can make sure that this won't happen by using singleshot QTimer
     * instead of a standard emit call */
    ...
    // lot in these battle functions We need to find zone of the destroyed card
    Zone *pZoneOfTheDestroyedCard =
        findZone(defenderZoneNumber, QString::fromStdString("monster card"),
...
void Game::battleBetweenTwoDifferentPositionMonsters(MonsterCard &attacker,
  ...
  if (defender.getPosition() == MonsterPosition::FACE_DOWN_DEFENSE) {
    ...
    outDataStream1
        ...
        << QString::fromStdString(
               defender.getCardName()) // TODO: This is maybe unneeded, since we
                                       // can locate the monster by just using
                                       // the zone number
...
auto Game::findZone(qint32 zoneNumber, QString cardType, Player *pTargetPlayer)
    -> Zone * {
...
void Game::firstTurnSetup(qint32 firstToPlay, qint32 clientID,
  ...
  ui->labelGamePhase->setText(QString::fromStdString(
      "DRAW PHASE")); // We hardcode it here since both clients are in sync here
  ...
  if (m_clientID == firstToPlay) {
    ...
    for (Card *card : GameExternVars::pOtherPlayer->m_hand.getHand()) {
      cardBackPixmap = cardBackPixmap.scaled(QSize(card->width, card->height),
                                             Qt::KeepAspectRatio);
  ...
  //                                             by 400 points and decreases its
  //                                             DEF by 200 points.",
  ...
  ui->labelCurrentPlayerLpDynamic->setText(QString::fromStdString(
      std::to_string(GameExternVars::pCurrentPlayer->getPlayerLifePoints())));
...
void Game::deserializeFieldPlacement(QDataStream &deserializationStream) {
  // TODO: There are still things here to be done, like reconstructing the cards
  // and summoning them in correct zone and position
  ...
  for (Card *card :
    ...
    if (cardName.toStdString() == card->getCardName()) {
      targetCard = card;
      GameExternVars::pCurrentPlayer->field.graveyard->removeFromGraveyard(
          *targetCard);
    }
  }
  ...
  if (cardType == "monster card") {
    ...
    GameExternVars::pCurrentPlayer->field.spellTrapZone.placeInSpellTrapZone(
        targetCard, zoneNumber);
  }
...
void Game::deserializeAddCardToHand(QDataStream &deserializationStream) {
  /* When player1 adds a card to the hand, game sends a header to the server to
     indicate that, and then we come here. In order for player2 (who is actually
     the CURRENT player while we are in this function) to changes in the
     opponent's (player1) hand, we just need to make other player draw one card.
...
void Game::onGameStart(qint32 firstToPlay, qint32 clientID) {
  ...
  clientID == firstToPlay ? m_clientName = QString::fromStdString(
                                GameExternVars::pCurrentPlayer->getPlayerName())
...
void Game::onEndPhaseButtonClick() {
  ...
  if(!GameExternVars::pCurrentPlayer || !GameExternVars::pOtherPlayer)
      //DO NOTHING
      return;
...
/* In order to have drag resizes on the main window, we can place CentralWidget
   in a layout, but weird things tend to happen when we actually resize then, so
   for now its not done like that.*/
void Game::onMainWindowResize(QResizeEvent *resizeEvent) {
  ...
  if (resizeCount == 2) {
    ...
    // std::cout << "New main window width/height: " << m_windowWidth << " / "
    ...
    // We need to calculate other UI sizes so we know what QGraphicsView's size
    // needs to be.
    ...
    background = background.scaled(m_viewAndSceneWidth, this->size().height(),
...
void Game::onCardHoverEnter(Card &card) {
  ...
  if (card.getCardType() == CardType::MONSTER_CARD) {
    ...
    atkDefIfMonster +=
        ...
        "  /  DEF: " + std::to_string(pMonsterCard->getDefensePoints());
...
void Game::onRepositionButtonClick(Card &card) {
  ...
  outDataStream
      ...
      << QString::fromStdString(
             monsterCard->getCardName()) // TODO: This is maybe unneeded, since
                                         // we can locate the monster by just
...
/* TODO: This should accept the targetPlayer as an argument, because otherwise
   we always print currentPlayer's hp, even if it was other player's health that
   had changed. */
...
void Game::onRedZoneClick(Zone *clickedRedZone) {
  ...
  if (card->getCardType() == CardType::MONSTER_CARD) {
    ...
    // If the position is SET, we need to rotate the card and change the pixmap
    // to card_back.jpg
    ...
    outDataStream.setVersion(QDataStream::Qt_5_15);
    outDataStream
        << QString("FIELD_PLACEMENT") // Header, so that server knows what data
                                      // to expect after it
        << QString::fromStdString(
               card->getCardName()) // We only send card's name, server will
                                    // pass it to the other client and then that
                                    // client will construct the card
...
void Game::onReturnCardToOpponent() {
  ...
  for (Zone *zone :
    ...
    if (zone->m_pCard == GameExternVars::pCardToBePlacedOnField) {
      GameExternVars::pCurrentPlayer->field.monsterZone.removeFromMonsterZone(
          zone);
      break;
    }
  }
