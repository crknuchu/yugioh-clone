...
Game::Game(Player p1, Player p2, int lifePoints, int numberOfCards, int timePerMove,
           QWidget* parent)
    : QMainWindow(parent), lifePoints(lifePoints), numberOfCards(numberOfCards),
      timePerMove(timePerMove), ui(new Ui::MainWindow), m_player1(p1), m_player2(p2),
...
void Game::setNumberOfCards(int newNumberOfCards) { numberOfCards = newNumberOfCards; }
...
void Game::switchPlayers() {
  ...
  // Switch clients too
  GameExternVars::currentTurnClientID == 1 ? GameExternVars::currentTurnClientID = 2
  ...
  outDataStream
      ...
      << GameExternVars::currentTurnClientID; // We tell the opposite player that his opponent
                                              // (current player in this call) got a card
...
void Game::battleBetweenTwoAttackPositionMonsters(MonsterCard& attacker, MonsterCard& defender) {
  ...
  if (attackPointsDifference < 0) {
    ...
    Zone* pZoneOfTheDestroyedCard = findZone(
        attackerZoneNumber, QString::fromStdString("monster card"), GameExternVars::pCurrentPlayer);
    ...
    outDataStream << QString::fromStdString(
     ...
     * We also have to use QEventLoop to make this client wait until client 2 sends
     * DESERIALIZATION_FINISHED (which will get sent to this client by the server) by making it loop
     * until its built-in slot quit() is triggered by our deserializationFinished signal.
     *
     *  IMPORTANT: In theory its possible that deserializationFinished is emitted before we enter
     * loop.exec(), which would leave us in an infinite loop, but that will practically never happen
     * since that signal is emitted after a lot of other operations (send to server -> relay to
     * another client -> send back -> emit) so we will always enter the loop before that.
     *
     *  TODO: We can make sure that this won't happen by using singleshot QTimer instead of a
     * standard emit call */
    ...
    // TODO: These 3 lines can be a separate function since they are reused a lot in these battle
    // functions We need to find zone of the destroyed card
    Zone* pZoneOfTheDestroyedCard = findZone(
        defenderZoneNumber, QString::fromStdString("monster card"), GameExternVars::pOtherPlayer);
    Card* pDestroyedCard = pZoneOfTheDestroyedCard->m_pCard;
    ...
    pZoneOfTheDestroyedCard = findZone(defenderZoneNumber, QString::fromStdString("monster card"),
...
void Game::battleBetweenTwoDifferentPositionMonsters(MonsterCard& attacker, MonsterCard& defender) {
  ...
  if (defender.getPosition() == MonsterPosition::FACE_DOWN_DEFENSE) {
    ...
    outDataStream1
        << QString::fromStdString("FLIP") << QString::fromStdString("OTHER_PLAYER")
        << QString::fromStdString(
               defender.getCardName()) // TODO: This is maybe unneeded, since we can locate the
                                       // monster by just using the zone number
    ...
    if (cardActivationReq) {
      ...
      outDataStream2 << QString::fromStdString("EFFECT_ACTIVATED")
                     ...
                     << QString::fromStdString(defender.getCardTypeString()) << defenderZoneNumber;
...
auto Game::findZone(qint32 zoneNumber, QString cardType, Player* pTargetPlayer) -> Zone* {
  ...
  cardType == "monster card" ? partOfField = pTargetPlayer->field.monsterZone.m_monsterZone
...
void Game::visuallyFlipSpell(SpellCard* pSpellCard) {
  // FIXME: We can't use this for both flipping our and opponent's cards, since our cards will be
  // rotated by 180 degress which is something we don't want to do.
  QPixmap pix;
...
void Game::firstTurnSetup(qint32 firstToPlay, qint32 clientID, float windowWidth,
                          float windowHeight) {

  std::cout << "Window width/height in firstTurnSetup: " << windowWidth << " / " << windowHeight
            << std::endl;
  ...
  std::cout << "The first one to play is " << GameExternVars::pCurrentPlayer->getPlayerName()
            << std::endl;
  m_currentTurn = 1;
  ...
  //                                               MonsterAttribute::EARTH, false,
  //                                               MonsterPosition::FACE_DOWN_DEFENSE, false,
  //                                               CardType::MONSTER_CARD, CardLocation::FIELD,
  //                                               "Neither player can target Dragon monsters on the
  //                                               field with card effects.",
  ...
  //                                               MonsterType::SPELLCASTER,
  //                                               MonsterKind::EFFECT_MONSTER,
  //                                               MonsterAttribute::DARK, false,
  //                                               MonsterPosition::ATTACK, false,
  ...
  //                                             SpellTrapPosition::NONE, "  An EARTH monster
  ...
  //                                               MonsterType::SPELLCASTER,
  //                                               MonsterKind::EFFECT_MONSTER,
  //                                               MonsterAttribute::EARTH, false, Position::ATTACK,
  //                                               false, CardType::MONSTER_CARD,
  //                                               CardLocation::FIELD, "Neither player can target
  //                                               Dragon monsters on the field with card effects.",
  //                                               ":/resources/pictures/HaneHane.jpg"
  //                                               );
  //   SpellCard* testCard2 = new SpellCard(SpellType::NORMAL_SPELL, "Change of Heart",
  //                                              CardType::SPELL_CARD, CardLocation::HAND,
  //                                              "  Increase your Life Points by 1000 points.  ",
  //                                              ":/resources/pictures/ChangeofHeart.jpg", true);

  //   MonsterCard* testCard3 = new MonsterCard("Blue dragon", 3000, 2500, 4,
  //                                               MonsterType::SPELLCASTER,
  //                                               MonsterKind::EFFECT_MONSTER,
  //                                               MonsterAttribute::DARK, false, Position::ATTACK,
  //                                               false, CardType::MONSTER_CARD,
  //                                               CardLocation::GRAVEYARD, "Neither player can
  //                                               target Dragon monsters on the field with card
  //                                               effects.",
  ...
  ui->labelOtherPlayerLpDynamic->setText(
      QString::fromStdString(std::to_string(GameExternVars::pOtherPlayer->getPlayerLifePoints())));
...
void Game::setupConnections() {
  ...
  connect(
      this, &Game::gameEndedAfterBattle, this,
      &Game::onGameEnd); // Same slot for both game endings (one in EffectActivator and one here)
...
auto Game::sendDataToServer(QByteArray& data) -> bool {
  if (m_pTcpSocket->state() == QAbstractSocket::ConnectedState) {
    ...
    std::cerr << "Error: the socket that is trying to write the data is in UNCONNECTED state!"
              << std::endl;
    return false;
  }
}
...
void Game::deserializeFieldPlacement(QDataStream& deserializationStream) {
  // TODO: There are still things here to be done, like reconstructing the cards and summoning them
  // in correct zone and position

  QString cardName;
  QString cardType;
  qint32 zoneNumber;
  QString positionQString;
  deserializationStream >> cardName >> cardType >> zoneNumber >> positionQString;
  ...
  originalPixmap =
      originalPixmap.scaled(QSize(targetCard->width, targetCard->height), Qt::KeepAspectRatio);
  ...
  if (cardType == "monster card") {
    ...
    GameExternVars::pCurrentPlayer->field.spellTrapZone.placeInSpellTrapZone(targetCard,
                                                                             zoneNumber);
  }
...
void Game::deserializeAddCardToHand(QDataStream& deserializationStream) {
  /* When player1 adds a card to the hand, game sends a header to the server to indicate that,
     ...
     TODO: We need to somehow ensure that, for example player1's, decks are in sync, in both game
     sessions/clients.       !!

      // We don't want this player to see the opponent's hand, so we could potentially change the
...
void Game::deserializeEffectActivated(QDataStream& deserializationStream) {
  ...
  QString cardType;
  qint32 zoneNumber;
  deserializationStream >> whichPlayerActivatedEffect >> cardName >> cardType >> zoneNumber;

  Card* targetCard;
  if (whichPlayerActivatedEffect == QString::fromStdString("CURRENT_PLAYER")) {
    cardType == "monster card"
        ? targetCard =
              GameExternVars::pCurrentPlayer->field.monsterZone.m_monsterZone[zoneNumber - 1]
                  ->m_pCard
        : targetCard =
              GameExternVars::pCurrentPlayer->field.spellTrapZone.m_spellTrapZone[zoneNumber - 1]
                  ->m_pCard;
  } else {
    cardType == "monster card"
        ? targetCard =
              GameExternVars::pOtherPlayer->field.monsterZone.m_monsterZone[zoneNumber - 1]->m_pCard
        : targetCard =
              GameExternVars::pOtherPlayer->field.spellTrapZone.m_spellTrapZone[zoneNumber - 1]
                  ->m_pCard;
  }
...
void Game::deserializeReposition(QDataStream& deserializationStream) {
  ...
  Card* targetCard =
      GameExternVars::pCurrentPlayer->field.monsterZone.m_monsterZone[zoneNumber - 1]->m_pCard;
...
void Game::deserializeDestroyCard(QDataStream& deserializationStream) {
  ...
  deserializationStream >> whoOwnedTheDestroyedCardQString >> cardType >> zoneNumber;
  ...
  pPlayerWhoOwnedTheDestroyedCard->sendToGraveyard(
      *pDestroyedCard,
      pZoneOfTheDestroyedCard); // TODO: Change all other occurences and add zone parameter
...
void Game::deserializeGameEnd(QDataStream& deserializationStream) {
  ...
  m_clientName == loserName ? QMessageBox::information(this, tr("GAME END"), tr("DEFEAT"))
...
void Game::onGameStart(qint32 firstToPlay, qint32 clientID) {
  ...
  GameExternVars::pCurrentPlayer->field.setField(flagBottomField, m_viewAndSceneWidth,
                                                 m_windowHeight);
  GameExternVars::pCurrentPlayer->m_hand.setHandCoordinates(m_viewAndSceneWidth, m_windowHeight,
                                                            flagBottomField);
  ...
  GameExternVars::pOtherPlayer->m_hand.setHandCoordinates(m_viewAndSceneWidth, m_windowHeight,
                                                          flagUpperField);
...
void Game::onGamePhaseChange(const GamePhases& newGamePhase) {
  ...
  QString currentGamePhase =
      GamePhaseExternVars::gamePhaseToQString.at(GamePhaseExternVars::currentGamePhase);
  std::cout << "Current game phase in onGamePhaseChange: " << currentGamePhase.toStdString()
            << std::endl;
  ...
  QDataStream outDataStream(&buffer, QIODevice::WriteOnly);
  outDataStream.setVersion(QDataStream::Qt_5_15);
  outDataStream << QString::fromStdString("GAMEPHASE_CHANGED") << currentGamePhase;
  sendDataToServer(buffer);
  blockingLoop.exec();
}
...
void Game::onTurnEnd() {
  ...
  if (m_clientID != GameExternVars::currentTurnClientID) {
    ...
    cardBackPixmap = cardBackPixmap.scaled(
        QSize(cardThatWeGotJustNow->width, cardThatWeGotJustNow->height), Qt::KeepAspectRatio);
...
/* In order to have drag resizes on the main window, we can place CentralWidget in a layout,
   but weird things tend to happen when we actually resize then, so for now its not done like
   that.*/
void Game::onMainWindowResize(QResizeEvent* resizeEvent) {
  /* TODO: Is there a way to make Qt only do one resize (immediately to the fullscreen resolution,
           instead of first resizing to the resolution that is same as in Designer (and only after
     that one doing a proper resize to fullscreen) */
  ...
  if (resizeCount == 2) {
    ...
    // std::cout << "New main window width/height: " << m_windowWidth << " / " << m_windowHeight <<
    // std::endl;
    ...
    background =
        background.scaled(m_viewAndSceneWidth, this->size().height(), Qt::IgnoreAspectRatio);
...
void Game::onCardHoverEnter(Card& card) {
  ...
  if (card.getCardType() == CardType::MONSTER_CARD) {
    ...
    atkDefIfMonster += "ATK: " + std::to_string(pMonsterCard->getAttackPoints()) +
  ...
  ui->textBrowserEffect->setText(
      QString::fromStdString(card.getCardDescription() + "\n\n\n" + atkDefIfMonster));
...
void Game::onCardSelect(Card* card) {
  ...
  // We need to check if the opponent has any monsters. If he doesn't, we don't show Attack Directly
  // button
...
void Game::onActivateButtonClick(Card& card) {
  ...
  if (card.getCardLocation() == CardLocation::HAND) {
    ...
    if (m_clientID == GameExternVars::currentTurnClientID) {
      ...
      for (Card* card : GameExternVars::pOtherPlayer->m_hand.getHand()) {
        cardBackPixmap =
            cardBackPixmap.scaled(QSize(card->width, card->height), Qt::KeepAspectRatio);
        card->setPixmap(cardBackPixmap);
      }
    } else {
      QPixmap cardBackPixmap(":/resources/pictures/card_back.jpg");
    ...
    if (card.shouldBeSentToGraveyard()) { // this needs to be implemented
      delay();
      card.getPlayerThatSetThisCard() == 1 ? GameExternVars::pCurrentPlayer->sendToGraveyard(card)
      ...
      outDataStream2.setVersion(QDataStream::Qt_5_15);
      outDataStream2
          << QString::fromStdString("DESTROY_CARD")   // Notify the server that a card was destroyed
          << QString::fromStdString("CURRENT_PLAYER") // Who owns a card that was destroyed
...
void Game::onSummonButtonClick(Card& card) {
  ...
  /* Set this card that is to-be-summoned to a global summon target, in order for Zone objects to be
     able to see it. */
  GameExternVars::pCardToBePlacedOnField = &card;
  std::cout << "Current summon target is: " << GameExternVars::pCardToBePlacedOnField->getCardName()
            << std::endl;

  // Set the monster's position explicitly to ATTACK (since thats the only one allowed when
  // summoning)
...
void Game::onRepositionButtonClick(Card& card) {
  ...
  // We are sure that this card is a MonsterCard since only monsters can change their position to
  // defense or attack
  auto* monsterCard = static_cast<MonsterCard*>(&card);
  ...
  outDataStream
      ...
      << QString::fromStdString(
             monsterCard->getCardName()) // TODO: This is maybe unneeded, since we can locate the
                                         // monster by just using the zone number
...
void Game::onFlipButtonClick(Card& card) {
  ...
  outDataStream.setVersion(QDataStream::Qt_5_15);
  outDataStream
      << QString::fromStdString("FLIP") << QString::fromStdString("CURRENT_PLAYER")
      << QString::fromStdString(
             monsterCard->getCardName()) // TODO: This is maybe unneeded, since we can locate the
                                         // monster by just using the zone number
      << zoneNumber;                     // Zone number, so we can locate the monster
...
/* TODO: This should accept the targetPlayer as an argument, because otherwise we always print
   currentPlayer's hp, even if it was other player's health that had changed. */
void Game::onLifePointsChange(Player& targetPlayer) // const?
{
  std::cout << "Current health points for player " << targetPlayer.getPlayerName() << " : "
...
void Game::onSetButtonClick(Card& card) {
  ...
  /* Set this card that is to-be-summoned to a global summon target, in order for Zone objects to be
     able to see it. */
  GameExternVars::pCardToBePlacedOnField = &card;
  std::cout << "Current Set target is: " << GameExternVars::pCardToBePlacedOnField->getCardName()
            << std::endl;
...
void Game::onRedZoneClick(Zone* clickedRedZone) {
  ...
  if (card->getCardType() == CardType::MONSTER_CARD) {
    ...
    auto* pMonsterCard = static_cast<MonsterCard*>(card);
    QString monsterPosition =
        pMonsterCard->monsterPositionEnumToQString.at(pMonsterCard->getPosition());
    ...
    outDataStream.setVersion(QDataStream::Qt_5_15);
    outDataStream
        << QString("FIELD_PLACEMENT") // Header, so that server knows what data to expect after it
        << QString::fromStdString(
               card->getCardName()) // We only send card's name, server will pass it to the other
                                    // client and then that client will construct the card
        << QString::fromStdString(card->getCardTypeString()) << zoneNumber << monsterPosition;
    sendDataToServer(buffer);
    blockingLoop.exec();
    ...
    if (card->getCardType() == CardType::SPELL_CARD) {
      auto* pSpellCard = static_cast<SpellCard*>(card);
      spellTrapPosition =
          pSpellCard->spellTrapPositionEnumToQString.at(pSpellCard->getSpellPosition());
      if (spellTrapPosition == QString::fromStdString("SET"))
        visuallySetSpell(pSpellCard);
    } else {
      auto* pTrapCard = static_cast<TrapCard*>(card);
      spellTrapPosition =
          pTrapCard->spellTrapPositionEnumToQString.at(pTrapCard->getTrapPosition());
    ...
    outDataStream << QString("FIELD_PLACEMENT") << QString::fromStdString(card->getCardName())
                  << QString::fromStdString(card->getCardTypeString()) << zoneNumber
                  << spellTrapPosition;
    sendDataToServer(buffer);
    blockingLoop.exec();
...
void Game::onNetworkErrorOccurred(QAbstractSocket::SocketError socketError) {
  ...
  default:
    QMessageBox::information(
        this, tr("Game client"),
        tr("The following error occurred: %1.").arg(m_pTcpSocket->errorString()));
  }
}
