...
SpellCard::SpellCard(SpellType type, const std::string& cardName, CardType cardType,
                     CardLocation cardLocation, SpellTrapPosition position,
                     const std::string& cardDescription, std::string imagePath, bool active)
    : Card(cardName, cardType, cardLocation, cardDescription, imagePath), type(type),
...
auto SpellCard::getSpellPosition() const -> SpellTrapPosition { return position; }
...
void SpellCard::setPosition(SpellTrapPosition newPosition) { position = newPosition; }
