...
EffectActivator::EffectActivator(Card &card) : m_card(&card) {}

EffectActivator::~EffectActivator() = default;

const std::map<std::string, EffectActivator::EFFECT_MEMBER_FUNCTION_POINTER>
    EffectActivator::effectMap = {
        // Monsters
        ...
        {"Dian Keto the Cure Master",
         &EffectActivator::activateDianKetoTheCureMaster},
        ...
        {"The Flute of Summoning Dragon",
         &EffectActivator::activateTheFluteOfSummoningDragon},
        {"The Inexperienced Spy",
         &EffectActivator::activateTheInexperiencedSpy},
        {"Sword Of Dark Destruction",
         &EffectActivator::activateSwordOfDarkDestruction},
...
auto EffectActivator::getCard() const -> Card * { return m_card; }

void EffectActivator::activateEffect(const std::string &cardName,
                                     bool isOpponentActivating) {
  try {
    auto effectFunctionPointer = effectMap.at(cardName);

    // (effectActivator.*funcPointer)(); // This is the same as the invoke call
    // below. If the first argument is a pointer to member func, invoke expects
    // an object that owns it to be a first argument.
    std::__invoke(effectFunctionPointer, this, isOpponentActivating);
  } catch (std::out_of_range &e) {
    std::cerr << "Error: That card doesn't have an effect! Out of range "
                 "exception from: "
              << e.what() << std::endl;
  }
}
void EffectActivator::setCard(Card &card) { m_card = &card; }
...
void EffectActivator::activateLordOfD(bool isOpponentActivating) {
  ...
  // This is actually a passive effect. Its only "activated" while Lord of D is
  // on the field.
  // TODO: How to make it apply only while Lord of D is on the field?
  // Serialization and deserialization ?
  makeMonstersOfThisTypeUntargetable(
      MonsterType::DRAGON, UntargetableBy::CARD_EFFECT, CardLocation::FIELD,
      *GameExternVars::pCurrentPlayer);

  // Test returnToHand
  returnToHand(*(this->getCard()), GamePhases::END_PHASE,
               *GameExternVars::pCurrentPlayer);
...
void EffectActivator::activateTheWickedWormBeast(bool isOpponentActivating) {
  Q_UNUSED(isOpponentActivating);
  std::cout << "The Wicked Worm Beast's effect has been activated!"
            << std::endl;
  // ...
}
  ...
  for (Zone *zone : monsters) {
    ...
    if (!zone->isEmpty() && m->getCardType() == CardType::MONSTER_CARD) {
      if (m->getMonsterType() == MonsterType::SPELLCASTER ||
          m->getMonsterType() == MonsterType::FIEND) {
...
void EffectActivator::activateBookOfSecretArts(bool isOpponentActivating) {
  ...
  strongestMonster->setDefensePoints(strongestMonster->getDefensePoints() +
                                     300);
}

void EffectActivator::activateHaneHane(bool isOpponentActivating) {
  MonsterCard *targetedMonster =
      findHighestATKMonster(*GameExternVars::pOtherPlayer);
  Zone *zoneWithTargetedMonster =
      GameExternVars::pOtherPlayer->field.monsterZone.getZone(targetedMonster);
  GameExternVars::pOtherPlayer->field.monsterZone.removeFromMonsterZone(
      zoneWithTargetedMonster);
...
void EffectActivator::activateSwordOfDarkDestruction(
    bool isOpponentActivating) {
  std::vector<Zone *> monsters =
      GameExternVars::pCurrentPlayer->field.monsterZone.m_monsterZone;
  std::cout << GameExternVars::pOtherPlayer->getPlayerName();
    ...
    for (Card *card :
      ...
      if (card->getCardType() == CardType::MONSTER_CARD) {
        ...
        if (!strongestMonsterInEitherGraveyard ||
            strongestMonsterInEitherGraveyard->getAttackPoints() <
                monsterInGraveyard->getAttackPoints())
...
void EffectActivator::activateChangeOfHeart(bool isOpponentActivating) {
  emit EffectActivator::effectChangeOfHeart(*GameExternVars::pCurrentPlayer,
...
// TODO: Methods (for example, setTargetable) used in these methods have to be
// implemented in the future.

// TODO: Variable arguments (1 or 2 players)
void EffectActivator::makeMonstersOfThisTypeUntargetable(
    const MonsterType &targetType, const UntargetableBy &restrictedMechanic,
    const CardLocation &cardLocation, Player &targetPlayer) {
  /* TODO: cout for these enums isn't possible because there are no extern enum
     -> string maps, so we can currently only cout the player that this applies
     to. */
  std::cout << "This applies to: " << targetPlayer << std::endl;

  // TODO: Normal naming system
  //    targetPlayer.setMonstersOfThisTypeUntargetable(targetType,
  //    restrictedMechanic, cardLocation, true);
}

void EffectActivator::returnToHand(Card &targetCard,
                                   const GamePhases &inWhichGamePhase,
                                   Player &targetPlayer) {
  std::cout << "Player " << targetPlayer.getPlayerName() << "'s card "
            << getCard()->getCardName()
            << " will be returned to the hand in the "
            << GamePhaseExternVars::gamePhaseToQString.at(inWhichGamePhase)
                   .toStdString()
            << "." << std::endl;
...
void EffectActivator::excavateCards(int numberOfCards, Player &targetPlayer) {
  ...
  for (int i = 0; i < numberOfCards; i++)
    excavatedCards[i] =
        targetPlayer.field.deck[i]; // TODO: Is there a better way?

  //    emit displayExcavatedCards(excavatedCards);
}

void EffectActivator::revealCardsInHand(int numberOfCards,
                                        Player &targetPlayer) {
  std::vector<Card *> hand = targetPlayer.m_hand.getHand();
  for (Card *card : hand)
    std::cout << *card << std::endl;
  if ((unsigned long)numberOfCards > hand.size()) {
    std::cerr << "Number of cards passed to revealCardsInHand() is bigger than "
   ...
   * In the future, there needs to be a system for opponent player to select
   * which ones he wants to reveal. It would probably be similar to our Attack
   * slot, where we color proper cards and wait for a click. */
...
auto EffectActivator::isCardOnField(Card &targetCard, Player &targetPlayer)
    -> bool {
  // TODO: MonsterZone, SpellTrapZone and other "zones" could have a method for
  // searching if card is in them?

  //    MonsterZone monsterZone(targetPlayer.monsterZone); // TODO: This way of
  //    copying vs vec1 = vec2 for(Zone *zone : monsterZone)
  //    {
  //        if(!zone->isEmpty())
  //        {
  ...
  // FIXME: This is only checking the MonsterZone, what if the card is a Spell
  // or a Trap...
}

auto EffectActivator::returnPlayerGraveyard(Player &targetPlayer)
    -> std::vector<Card *> {
  return targetPlayer.field.graveyard->getGraveyard();
}

void EffectActivator::increaseATK(MonsterCard &targetCard, int increaseBy) {
  // TODO: Currently there is an implicit cast from unsigned to int, maybe
  // change it in Card class
...
auto EffectActivator::findLowestATKMonsters(Player &targetPlayer)
    -> std::vector<MonsterCard *> {
  // TODO: A more functional way (copy_if) for this

  //    MonsterZone monsterZone(targetPlayer.monsterZone); // TODO: This way of
  //    copying vs vec1 = vec2
...
auto EffectActivator::findHighestATKMonster(Player &targetPlayer)
    -> MonsterCard * {
  ...
  for (Zone *zone : targetPlayer.field.monsterZone.m_monsterZone) {
    if (!zone->isEmpty()) {
      ...
      if (!highestATKMonster ||
          monster->getAttackPoints() > highestATKMonster->getAttackPoints())
...
auto EffectActivator::findFaceUpMonsters(Player &targetPlayer)
    -> std::vector<MonsterCard *> {
  //    MonsterZone monsterZone(targetPlayer.monsterZone); // Copy constructor
  //    vs =
...
void EffectActivator::changeHealthPointsBy(int pointChange,
                                           Player &targetPlayer) {
  unsigned currentPlayerLifePoints = targetPlayer.getPlayerLifePoints();

  std::cout << "currentPlayerLifePoints: " << currentPlayerLifePoints
            << std::endl;

  // This shouldn't be unsigned because it goes to UINT_MAX if its subtracted
  // below 0.
  int newHealthPoints = currentPlayerLifePoints + pointChange;
  newHealthPoints > 0 ? targetPlayer.setPlayerLifePoints(newHealthPoints)
                      : emit gameEnded(targetPlayer);

  // The following code is only for the purpose of unified output, it can be
  // made prettier probably.
  std::string lostOrGained = pointChange < 0 ? "lost" : "gained";

  // We want to have non-negative points for the output, since we already say
  // "lost" or "gained" it would be weird to say lost -500 hp.
  pointChange = pointChange < 0 ? pointChange * -1 : pointChange;

  std::cout << "Player " << targetPlayer.getPlayerName() << " " << lostOrGained
            << " " << pointChange << " health points." << std::endl;
