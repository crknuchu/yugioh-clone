...
void EffectActivator::activateEffect(const std::string& cardName, bool isOpponentActivating) {
  try {
    ...
    // If the first argument is a pointer to member func, invoke expects an object that owns it to
    // be a first argument.
    std::invoke(effectFunctionPointer, this, isOpponentActivating);
  } catch (std::out_of_range& e) {
    std::cerr << "Error: That card doesn't have an effect! Out of range exception from: "
              << e.what() << std::endl;
  }
}
...
void EffectActivator::activateLordOfD(bool isOpponentActivating) {
  ...
  makeMonstersOfThisTypeUntargetable(MonsterType::DRAGON, UntargetableBy::CARD_EFFECT,
                                     CardLocation::FIELD, *GameExternVars::pCurrentPlayer);
...
void EffectActivator::activateSogen(bool isOpponentActivating) { Q_UNUSED(isOpponentActivating); }
...
void EffectActivator::activateDeSpell(bool isOpponentActivating) { Q_UNUSED(isOpponentActivating); }
...
// TODO: Methods (for example, setTargetable) used in these methods have to be implemented in the
// future.

// TODO: Variable arguments (1 or 2 players)
void EffectActivator::makeMonstersOfThisTypeUntargetable(const MonsterType& targetType,
                                                         const UntargetableBy& restrictedMechanic,
  ...
  //    targetPlayer.setMonstersOfThisTypeUntargetable(targetType, restrictedMechanic, cardLocation,
  //    true);
}

void EffectActivator::returnToHand(Card& targetCard, const GamePhases& inWhichGamePhase,
                                   Player& targetPlayer) {
  std::cout << "Player " << targetPlayer.getPlayerName() << "'s card " << getCard()->getCardName()
            << " will be returned to the hand in the "
            << GamePhaseExternVars::gamePhaseToQString.at(inWhichGamePhase).toStdString() << "."
            << std::endl;
...
void EffectActivator::revealCardsInHand(int numberOfCards, Player& targetPlayer) {
  ...
  if ((unsigned long)numberOfCards > hand.size()) {
    std::cerr << "Number of cards passed to revealCardsInHand() is bigger than the actual number "
   ...
   * In the future, there needs to be a system for opponent player to select which ones he wants to
   * reveal. It would probably be similar to our Attack slot, where we color proper cards and wait
   * for a click. */
...
auto EffectActivator::isCardOnField(Card& targetCard, Player& targetPlayer) -> bool {
  // TODO: MonsterZone, SpellTrapZone and other "zones" could have a method for searching if card is
  // in them?

  //    MonsterZone monsterZone(targetPlayer.monsterZone); // TODO: This way of copying vs vec1 =
  //    vec2 for(Zone *zone : monsterZone)
  //    {
  //        if(!zone->isEmpty())
  //        {
...
void EffectActivator::changeHealthPointsBy(int pointChange, Player& targetPlayer) {
  ...
  // We want to have non-negative points for the output, since we already say "lost" or "gained" it
  // would be weird to say lost -500 hp.
