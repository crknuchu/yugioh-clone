...
Player::Player(std::string playerName, int points)
    : m_hand(Hand()), field(Field()), m_name(std::move(playerName)),
      m_points(points){};
...
void Player::drawCards(unsigned int numOfCards) {
  ...
  if (this->field.deck.getDeck().empty() == true || deckSize < numOfCards) {
    ...
    std::cout << "The player " << this->getPlayerName() << " gets "
              << newCards.size() << " cards." << std::endl;
  }
}
...
void Player::fromGraveyardToField(Card &card, int zoneNumber) {
  if (isCardInGrave(card) == true) {
    this->field.graveyard->removeFromGraveyard(
        card); // this function return removed card,
...
void Player::sendToGraveyard(Card &card, Zone *zone) {
  zone->m_pCard = nullptr; // free space for that zone, card is sent to
                           // graveyard ===> zone.isEmpty() returns true after
  ...
  try {
    ...
    for (auto it : this->field.monsterZone.m_monsterZone) {
      if ((*it).m_pCard == &card) {
        ...
        this->field.graveyard->sendToGraveyard(card);
        std::cout << (*it).m_pCard->getCardName()
                  << " successfully removed from monsterZone" << std::endl;
        return;
      }
      position++;
    }
    ...
    for (auto it : this->m_hand.getHand()) {
      if ((*it) == card) {
        ...
        this->field.graveyard->sendToGraveyard(card);
        std::cout << card.getCardName() << " succesfully removed from hand"
                  << std::endl;
        return;
      }
    }
...
auto Player::checkOpponentGround(Player &opponent) -> int {
  std::vector<Zone *> cards =
      opponent.field.monsterZone
          .m_monsterZone; // first monsterZone is field in player.h
...
auto operator<<(std::ostream &out, Player &p) -> std::ostream & {
  return out << "Player name: " << p.getPlayerName() << ", points "
             << p.getPlayerLifePoints() << std::endl;
}
