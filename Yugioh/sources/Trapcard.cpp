...
TrapCard::TrapCard(TrapType type, const std::string &cardName,
                   ...
                   bool active, bool setThisTurn)
    : Card(cardName, cardType, cardLocation, cardDescription, imagePath),
      trapType(type), active(active), position(position) {}

auto TrapCard::clone() -> TrapCard * {
  return new TrapCard(this->trapType, this->cardName, this->cardType,
                      this->cardLocation, this->position, this->cardDescription,
...
auto TrapCard::getTrapTypeString() const -> std::string {
  switch (trapType) {
  case TrapType::NORMAL_TRAP: return "normal trap";
  case TrapType::CONTINUOUS_TRAP: return "constinious trap";
  case TrapType::COUNTER_TRAP: return "counter trap";
  default: return "error:unknown trap type";
  }
}
